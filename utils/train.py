...
def train():
  batch_size = 32
  location = os.path.realpath(os.getcwd() + '/../dataset/')
  label_mode = 'binary'
...
if __name__ == "__main__":
  history = train()   # revised #
  plt.plot(history.history['loss'])     # added
  plt.plot(history.history['acc'])     # added
  plt.title('acc loss vs epoch')     # added
  plt.xlabel('epoch')     # added
  plt.legend(['loss', 'acc'], loc='upper left')     # added
  plt.show()     # added
